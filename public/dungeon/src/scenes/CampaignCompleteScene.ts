...
export class CampaignCompleteScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.add
      .text(480, 130, `${modeLabelTitle(mode)} Complete`, {
        fontSize: '22px',
        color: '#e0e0ea',
        fontFamily: 'monospace',
      })
    ...
    if (nextSave.title_earned && !save.title_earned) {
      this.add
        .text(480, 180, `You have earned the title: ${nextSave.title_earned}`, {
          fontSize: '18px',
          color: '#ffca28',
          fontFamily: 'monospace',
          fontStyle: 'italic',
      ...
      this.add
        .text(480, 180, `Title: ${nextSave.title_earned}`, {
          fontSize: '16px',
          color: '#f5e4b3',
          fontFamily: 'monospace',
        })
    ...
    if (newUnlocks.length > 0) {
      this.add
        .text(480, 470, `\u2728 New spell unlocked: ${newUnlocks.join(', ')}`, {
          fontSize: '16px',
          color: '#ffca28',
          fontFamily: 'monospace',
        })
    ...
    if (!save.eternal_dungeon_unlocked && nextSave.eternal_dungeon_unlocked) {
      this.add
        .text(480, 500, '\uD83D\uDDDD Eternal Dungeon unlocked!', {
          fontSize: '16px',
          color: '#8bc34a',
          fontFamily: 'monospace',
        })
    ...
    this.add
      .text(480, 540, '\u2B07 Download session log (JSON)', {
        fontSize: '15px',
        color: '#d0d0da',
        fontFamily: 'monospace',
      })
    ...
    dlBtn.on(
      'pointerdown',
      (_: unknown, _x: number, _y: number, event: Phaser.Types.Input.EventData) => {
    ...
    this.add
      .text(480, 590, '(press Space / Enter to return to Hub)', {
        fontSize: '14px',
        color: '#a0a0b0',
        fontFamily: 'monospace',
      })
