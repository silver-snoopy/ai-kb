...
function makePanelFake() {
  ...
  return {
    state,
    setFillStyle(fill: number) {
      state.fill = fill;
      return this;
    },
    setStrokeStyle(w: number, color: number) {
      state.strokeWidth = w;
...
describe('paintOptionFeedback', () => {
  it('marks correct option green with ✓ and chosen-wrong option red with ✗', () => {
    ...
    paintOptionFeedback(
      panels as unknown as Phaser.GameObjects.Rectangle[],
      texts as unknown as Phaser.GameObjects.Text[],
      'C',
      'A',
    );
...
D: Wrong, no such ParallelTask tool exists.`;
  ...
  it('falls back to length-truncation when no per-option structure', () => {
    ...
    expect(summarizeExplanation(exp, 'A')).toBe(
      'Plain prose explanation without letter markers that is short.',
    );
  });
