...
const L = (
  text: string,
  trigger: NarratorLine['trigger'],
  bossId?: NarratorLine['bossId'],
): NarratorLine => ({ text, trigger, bossId });

describe('LinePool', () => {
  it('returns a line matching the trigger', () => {
    const pool = new LinePool([
      L('A', 'battle-start', 'the-orchestrator'),
      L('B', 'phase-66', 'the-orchestrator'),
    ]);
  ...
  it('prefers boss-specific over generic when both exist', () => {
    const pool = new LinePool([
      L('generic', 'filler'),
      L('specific', 'battle-start', 'the-orchestrator'),
    ]);
