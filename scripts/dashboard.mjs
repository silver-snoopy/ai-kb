...
async function renderCertBlock(vaultRoot, cert) {
  ...
  const countdownLine =
    days != null ? `**${name}** \u2014 ${days} days to exam (target: ${target})` : `**${name}**`;
  ...
  lines.push('');
  lines.push(
    `_Weakness queue for ${certId}: ${weakness.unchecked} unchecked / ${weakness.total} total_`,
  );
  lines.push('');
  ...
  if (exams.length) {
    ...
    for (const e of exams.slice(-5)) {
      ...
      lines.push(
        `- ${e.date ?? '?'} \u00b7 ${e.mode ?? '?'} \u00b7 ${e.score}/${e.total} (${pct.toFixed(0)}%) \u00b7 ${verdict}`,
      );
    }
